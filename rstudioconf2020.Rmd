...
`r emo::ji("point_right")` allows for nice tab completion!
...
# A generic formatter: `label_number`

+ This generic formatter offers consistency in behaviour and arguments across scales' many other formatters and offers additional functionality for international customization.
...
`label_*` functions output a function by default. 
...
By default, `label_number()` forces decimal displays of numbers, `label_comma()`, `label_dollar`, and `label_percent()` are special cases of `label_number()` with different defaults. 

```{r}
label_number()(c(12.3, 4, 12345.789, 0.0002))
```

You can easily specify a different rounding behavior, or change the `big_mark` or `decimal_mark` for international styling. Even add a `prefix` or a `suffix` or `scale` your numbers on the fly. 

```{r}
label_number(big.mark = ".",
  decimal.mark = ",", accuracy = .0001)(c(12.3, 4, 12345.789, 0.0002))
...
```{r}
dates <- txhousing$date %>% lubridate::date_decimal() %>%
  lubridate::round_date("day")

demo_datetime(dates)
demo_datetime(dates, breaks = breaks_width("2 year"), labels = label_date_short())
